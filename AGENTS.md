...
## BrainSAIT operations (2026-08-19)

### Store & credentials
- `brainsait.myshopify.com` = `f3rbxp-n1.myshopify.com` = `store.brainsait.org` (same store). `brainsait-theme.myshopify.com` DOES NOT EXIST — never use it.
- `account.brainsait.org` is the customer-account PORTAL SUBDOMAIN of the main store (NOT a separate store): CNAME `shops.myshopify.com`, must be DNS-only (Cloudflare proxy OFF — Shopify rejects proxied account domains), TXT `shopify_verification_account.brainsait.org` = `4T6AULJF` exists. Set as primary customer-account domain 2026-08-19; portal provisioning can take up to 48h (host answers 406 until then). The separate store `account-brainsait-org-xdzsyia9.myshopify.com` / `brainsait-store.myshopify.com` (password-protected "Account.brainsait.org") is ORPHANED — portals live on the main store.
- Admin token lives in `/home/fadil369/brainsait-shopify-app/.env` (`SHOPIFY_ADMIN_TOKEN`); read it from there, never hardcode. Admin token LACKS `read_collections` scope — use Storefront API via hub for collections. Metafield upsert: PUT on resource-scoped paths 406s — use `POST /admin/api/2025-10/products/{id}/metafields.json`. GET `metafields.json` (global) lists SHOP metafields only — use `/products/{id}/metafields.json` or GraphQL per product.

### Deploy
- Theme dir is NOT a git repo. Push directly to the live theme:
  `shopify theme push --store brainsait.myshopify.com --password "$(grep '^SHOPIFY_ADMIN_TOKEN=' ../brainsait-shopify-app/.env | cut -d= -f2)" --theme 149209940051 --allow-live`
- Live theme id: **149209940051** ("Super Store Theme"). `shopify theme list` wraps long ids — get exact ids from Admin API if in doubt. Unused duplicates: 149221933139 (trust-bar theme, content merged into live — deletable), 148931149907, 148731494483.
- Lint: `shopify theme check --path .` must report 0 offenses before pushing.
- WARNING: Do NOT rename/publish themes via the Admin REST `themes/{id}.json` PATCH/PUT. This store's published-theme role is **`main`** (not `live`); a bare `{"theme":{"name":...}}` PATCH silently demotes the live theme to `unpublished` and can promote another unpublished theme to `main`, breaking the store. Always use `shopify theme push --allow-live` (or set role `"main"` explicitly). If the live theme ever goes `unpublished`, PUT `themes/{id}.json` with `{"theme":{"role":"main"}}` to restore.

### Theme conventions (enforced by theme check)
- NEVER put Liquid variables inside `{% javascript %}` tags. Use data attributes for translations (`data-add-text="{{ 'products.add_to_cart' | t }}"`) and `<script type="application/json">` for product JSON.
- Every user-facing string must be in `locales/en.default.json` (+ `.schema.json` for editor labels), sentence case.
- Snippets require a `{% doc %}` header. Use `{% stylesheet %}`/`{% javascript %}` per component.

### Catalog & theme state (2026-08-19)
- Catalog: learn 40 (priced), build = 4 installment tickets (full 9630 / half 4815 / third 3210 / quarter 2407.50) + 6 draft courses, solutions 53 (39 reactivated at SAR 0 + `custom.booking_url` metafield = Google Calendar appointment URL; 13 priced untouched; 1 archived duplicate "BrainSAIT eCarePlus Portal").
- SOLUTIONS product pages: `sections/product.liquid` + `snippets/product-card.liquid` render "Book a session" CTA (keys `products.book_a_session` / `products.book_note`) instead of price/buy form when `product.metafields.custom.booking_url` exists.
- Product template (`templates/product.json`) includes trust bar section `17871298323eadb0fe` → block `blocks/ai_gen_block_de61bb3.liquid` (navy, 4 items: Instant Digital Delivery / Secure Checkout / Arabic & English / Lifetime Access).
- BUILD Ticket — Incubation Pass: product 8046917353555, variant Standard `45947217870931` (SKU `BSP-BUILD-TICKET-IDENTITY`, 9630 SAR, inventory now 0 with `continue` policy, tracking enabled).
- Arabic: storefront default locale is **ar-SA** — user-facing strings must also be added to `locales/ar.json` (currently fully translated). Fresh URLs (`/en/products/...` or never-fetched handles) render current theme; previously-fetched URLs may serve Shopify edge full-page cache for a while — never treat stale HTML as a code bug.

### UCP
- Store serves UCP natively at `store.brainsait.org/.well-known/ucp`; MCP at `f3rbxp-n1.myshopify.com/api/ucp/mcp` (private storefront token header). All calls need `meta: { "ucp-agent": { profile: "https://app.brainsait.de/ucp-agent.json" } }`; `search_catalog` args nest query under `catalog`; cart/checkout ids are the top-level `id` of the response payload.
- Agent profile: `https://app.brainsait.de/ucp-agent.json`; theme exposes it via `rel="ucp-agent"` link (setting `ucp_agent_url`). Local DNS gotcha: systemd-resolved (via Tailscale) caches stale IPs for `app.brainsait.de` (Hetzner 46.62.128.198 is authoritative) — fix with `resolvectl flush-caches`.
- Full memory: `/home/fadil369/brainsait-shopify-app/docs/UCP-SETUP-MEMORY.md` + BrainSAIT KB doc `brainsait-shopify-ucp-setup`.

## BrainSAIT operations (2026-08-20)

- Theme REBUILT + DEPLOYED to live theme 149209940051 (--allow-live): new gh design system (gh-skin.css), sections gh-home/gh-shelf/gh-build/gh-track, snippets/gh-pcard, gh-build.js/gh-track.js. `shopify theme check --path .` = 0 offenses (note: ValidScopedCSSClass has NO config option; use `{% # theme-check-disable ValidScopedCSSClass %}` comments; `.theme-check.yml` `ignore:` must be the LEGACY LIST format `- path` — the file→checks MAP format crashes the bundled CLI with "n.replace is not a function"; full check takes ~3-4 min due to gh-skin.css). Repo pushed: github.com/Fadil369/brainsait-theme (public). Theme runs ~4x slower lint.
- product.json on live INCLUDES trust-bar section 17871298323eadb0fe (block ai_gen_block_de61bb3, navy, 4 items) — restored from zip after first push dropped it; block file copied into blocks/.
- /pages/build + /pages/track DON'T EXIST (404). Creating them needs read_content/write_content. The brainsait-shopify-app token lacks content scopes; hub token too (Access denied for pages field). App 213e65fdc5e1053592eba8caee5729c1 (shpss_f27d...) = NEW app identity: valid but app_not_installed on main store; its registered App URL is NOT app.brainsait.de (OAuth error invalid_request "redirect_uri and application url must have matching hosts") — App URL unknown, install blocked. REDACTED-SHOPIFY-TOKEN + REDACTED-SHOPIFY-TOKEN + 674e89799ba28f7d3cad601757e596be + Hydrogen env ALL belong to ORPHANED store account-brainsait-org-xdzsyia9 ("Account.brainsait.org") — verified per-token. MAIN store still has NO content-scoped token.
- brainsait-shopify-app .env SWAPPED to new app identity (SHOPIFY_API_KEY=213e65..., secret shpss_f27d..., SCOPES += read_content,write_content; backup .env.bak-20260820). CONSEQUENCE: main-store app session/webhooks are orphaned until the new app is installed on the main store (needs its registered redirect URI); revert from backup if install can't be completed.
- Hydrogen storefront scaffolded at /home/fadil369/brainsait-hydrogen (skeleton template, workspace:* deps pinned to registry: @shopify/hydrogen 2026.4.5, mini-oxygen 4.2.2, hydrogen-codegen 0.3.3; .env from user config incl. session secret cac39ae...). Dev server verified: renders "Account.brainsait.org" from Storefront API (products empty — orphan store has no published products). Port 3002. `npx shopify hydrogen link --storefront "Brainsait-store"` + `hydrogen deploy` still need interactive CLI login.
- brainsait-store.myshopify.com = password-gated ("Account.brainsait.org"). Bot protection (web-bot-auth): passing the user's sig1 as `Signature:` header + `signature-agent: browser` clears it (401→302), but password POST (odrish) never sets an auth cookie — store access unverified; Hydrogen dev URL brainsait-store-a55900af4300128cf7e9.o2.myshopify.dev = 404.
- `shopify theme check` ValidScopedCSSClass: 10 bs-* sections have `{% # theme-check-disable ValidScopedCSSClass %}` (page-width is a Dawn global class).
